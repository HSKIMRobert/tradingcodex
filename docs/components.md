...
| `context-efficiency-contract` | Keeps workflows bounded through compact briefs, artifact references, context summaries, source snapshot IDs, and targeted full-artifact reads. | `guardrail.guidance`, `guardrail.information_barrier`, `improvement.workflow_quality`, `improvement.context_efficiency` |
...
| `tcx-source-gate` | Routes existing Snapshot/Dataset → one relevant user capability → optional direct OpenBB → original public records → reliable web → an explicit gap, and preserves source-aware evidence. | `guardrail.guidance`, `improvement.workflow_quality` |
