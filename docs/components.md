...
| `tcx-source-gate` | Assigns one DataNeed owner, routes Dataset → enabled user capability → supported OpenBB → official/web fallback, preserves typed attempts and raw rows, and keeps external evidence read-only and source-aware. | `guardrail.guidance`, `improvement.workflow_quality` |
