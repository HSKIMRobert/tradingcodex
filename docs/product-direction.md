...
| Deterministic executable boundary | Admit final submit/cancel only through an exact action-only immediate root-native prompt or an exact first-meaningful-line `$tcx-order-allow` current-turn grant with hook-injected proof. Presentation normalization and a matching projected skill link do not relax the action or mode grammar. Both paths retain service checks for requester identity, permission, policy fit, payload shape, exact approval, duplicate-request state, connection, live confirmation, and audit trail. |
