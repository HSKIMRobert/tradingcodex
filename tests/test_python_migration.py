...
def workflow_plan_draft(intake: dict, roles: list[str] | None = None) -> dict:
    candidates = intake.get("deterministic_hint", {}).get("roles") or intake.get("heuristic_roles") or []
    return {"workflow_run_id": intake["workflow_run_id"], "selected_roles": list(candidates if roles is None else roles)}
...
def test_package_wheel_includes_web_static_assets(tmp_path: Path) -> None:
    ...
    shutil.copytree(
        ROOT,
        source,
        ignore=shutil.ignore_patterns(".git", ".pytest_cache", "__pycache__", "*.pyc", "node_modules", "dist", "build", "*.egg-info"),
    )
    wheel_dir.mkdir()
    ...
    assert "tradingcodex_service/static/tradingcodex_web/index.html" in names
    ...
    assert "tradingcodex_service/static/vendor/htmx/htmx.min.js" not in names
...
def test_file_native_agent_skill_registry_contract() -> None:
    ...
    assert len(EXPECTED_SKILLS) == 28
    assert "decision-memory" in EXPECTED_SKILLS
    assert "investor-context" in EXPECTED_SKILLS
...
def test_user_prompt_hook_auto_routes_plain_investment_requests(tmp_path: Path) -> None:
    ...
    assert "workflow_run_id and selected_roles" in direct_gate["planning_instruction"]
    assert "draft, validate" not in direct_gate["planning_instruction"]
    ...
    run(
        [
            ...
            "strategies",
            "create",
            "strategy-hook-explicit",
            "--description",
            "Use explicit hook strategy selection.",
            "--active",
        ],
        workspace,
    )
    strategy_gate = run_user_prompt_hook(
        workspace,
        "$strategy-hook-explicit analyze Apple stock. No order or trading.",
    )
    assert strategy_gate
    strategy_intake = json.loads(
        (workspace / ".tradingcodex" / "mainagent" / "latest-workflow-intake.json").read_text(encoding="utf-8")
    )
    strategy_binding = strategy_intake["strategy_binding"]
    assert strategy_binding["strategy_id"] == "strategy-hook-explicit"
    assert strategy_binding["snapshot_path"].endswith("/strategy-snapshot.md")
    strategy_snapshot = workspace / strategy_binding["snapshot_path"]
    strategy_source = workspace / strategy_binding["source_file"]
    assert strategy_snapshot.read_bytes() == strategy_source.read_bytes()
    assert hashlib.sha256(strategy_snapshot.read_bytes()).hexdigest() == strategy_binding["content_hash"]

    natural_strategy_gate = run_user_prompt_hook(
        workspace,
        "Analyze Apple stock using strategy-hook-explicit. No order or trading.",
    )
    assert natural_strategy_gate
    natural_strategy_intake = json.loads(
        (workspace / ".tradingcodex" / "mainagent" / "latest-workflow-intake.json").read_text(encoding="utf-8")
    )
    assert natural_strategy_intake["strategy_binding"]["strategy_id"] == ""
    assert natural_strategy_intake["strategy_binding"]["snapshot_path"] == ""
    ...
    allowed_plan_mcp = run(
        [str(workspace / ".codex" / "hooks" / "tradingcodex_hook.py"), "pre-tool-use"],
        workspace,
        input_text=json.dumps({
            "tool_name": "mcp__tradingcodex__record_workflow_plan",
            "tool_input": {"plan": {"blocked_actions": ["direct broker API"]}},
        }),
    )
    assert allowed_plan_mcp.stdout.strip() == ""
    blocked_external_plan_mcp = run(
        [str(workspace / ".codex" / "hooks" / "tradingcodex_hook.py"), "pre-tool-use"],
        workspace,
        input_text=json.dumps({
            "tool_name": "mcp__untrusted__record_workflow_plan",
            "tool_input": {"plan": {"blocked_actions": ["direct broker API"]}},
        }),
    )
    assert json.loads(blocked_external_plan_mcp.stdout)["decision"] == "block"
    ...
    profile_context_binding = profile_aware_persisted_intake["investor_context_binding"]
    assert profile_context_binding["applied"] is True
    assert profile_context_binding["snapshot_path"].endswith("/investor-context-snapshot.md")
    assert (workspace / profile_context_binding["snapshot_path"]).read_bytes() == (
        workspace / profile_context_binding["path"]
    ).read_bytes()
...
def test_python_generator_creates_workspace_contract(tmp_path: Path) -> None:
    ...
    assert workspace_manifest["active_profile"]["label"] == "isolated workspace paper account"
    ...
    assert len(skill_index["skills"]) == 28
    ...
    assert skill_index["skills"]["decision-memory"]["user_visible"] is True
    assert skill_index["skills"]["investor-context"]["user_visible"] is True
    assert skill_index["skills"]["decision-memory"]["implicit_invocation"] is False
    assert skill_index["skills"]["investor-context"]["implicit_invocation"] is False
    assert skill_index["skills"]["postmortem"]["user_visible"] is False
    ...
    assert "integrity-bound `deterministic_hint` fixes the lane" in workflow_guidance
    assert "The server builds the stage DAG and owns constraints" in workflow_guidance
    ...
    assert len(subagent_metadata) + len(skill_index["skills"]) >= 28
    assert not (workspace / ".tradingcodex" / "user" / "profile.md").exists()
    assert not (workspace / ".tradingcodex" / "user" / "investor-context.md").exists()
    ...
    assert profile_status["active_profile"]["label"] == "isolated workspace paper account"
    ...
    assert "TradingCodex MCP workspace binding configured" in doctor
    ...
    expected_subagent_matcher = set(EXPECTED_SUBAGENTS)
    assert set(hooks["SubagentStart"][0]["matcher"].split("|")) == expected_subagent_matcher
    assert set(hooks["SubagentStop"][0]["matcher"].split("|")) == expected_subagent_matcher
    principals = yaml.safe_load((workspace / ".tradingcodex/policies/principals.yaml").read_text(encoding="utf-8"))["principals"]
    bindings = yaml.safe_load((workspace / ".tradingcodex/policies/policy-bindings.yaml").read_text(encoding="utf-8"))["bindings"]
    assert {principal["id"] for principal in principals} == set(AGENT_SPECS)
    assert set(bindings) == set(AGENT_SPECS)
    ...
    assert "Do not open the workbench unless the user asks" in head_manager_instructions
    ...
    assert "decision-memory/SKILL.md" in root_config_text
    ...
    assert "investor-context/SKILL.md" in root_config_text
    ...
    role_base_rules = root_config["permissions"]["tradingcodex-role-base"]["filesystem"][":workspace_roots"]
    assert role_base_rules[".tradingcodex/user/**"] == "deny"
    for profile_name in [
        ...
        assert filesystem_rules[".agents/skills/decision-memory/**"] == "deny"
        assert filesystem_rules[".agents/skills/automate-workflow/**"] == "deny"
        assert filesystem_rules[".agents/skills/investor-context/**"] == "deny"
    ...
    assert root_mcp["cwd"] == "."
    assert (workspace / root_mcp["cwd"]).resolve() == workspace.resolve()
    ...
    for agent_file in agent_files:
        ...
        assert (workspace / agent_mcp["cwd"]).resolve() == workspace.resolve()
    ...
    assert len(run(["./tcx", "skills", "list", "--all"], workspace).stdout.splitlines()) == 28
...
def test_model_policy_projects_gpt56_and_fails_closed_without_it(monkeypatch, tmp_path: Path) -> None:
    ...
    root_config = tomllib.loads((active_workspace / ".codex/config.toml").read_text(encoding="utf-8"))
    assert root_config["model"] == "gpt-5.6-sol"
    assert root_config["model_reasoning_effort"] == "xhigh"
    active_manifest = json.loads((active_workspace / ".tradingcodex/generated/model-policy-manifest.json").read_text(encoding="utf-8"))
    assert {item["resolved_model"] for item in active_manifest["roles"].values()} == {"gpt-5.6-sol", "gpt-5.6-terra"}
    assert "rollout" not in active_manifest
    assert "optional_feature_policy" not in active_manifest
    for role, policy in active_manifest["roles"].items():
        assert "fallback_models" not in policy
        assert "rollback_target" not in policy
        if role == "head-manager":
            assert (policy["resolved_model"], policy["reasoning_effort"]) == ("gpt-5.6-sol", "xhigh")
        elif role == "execution-operator":
            assert (policy["resolved_model"], policy["reasoning_effort"]) == ("gpt-5.6-terra", "low")
        else:
            assert (policy["resolved_model"], policy["reasoning_effort"]) == ("gpt-5.6-terra", "high")

    monkeypatch.setenv("TRADINGCODEX_CODEX_SUPPORTED_MODELS", "gpt-5.6-sol,gpt-5.6-terra")
    verified_workspace = make_workspace(tmp_path / "verified")
    verified_manifest = json.loads((verified_workspace / ".tradingcodex/generated/model-policy-manifest.json").read_text(encoding="utf-8"))
    assert {item["support_status"] for item in verified_manifest["roles"].values()} == {"verified"}

    monkeypatch.setenv("TRADINGCODEX_CODEX_SUPPORTED_MODELS", "gpt-5.5")
    with pytest.raises(ValueError, match="required Codex model is unavailable"):
        make_workspace(tmp_path / "unsupported")

    monkeypatch.delenv("TRADINGCODEX_CODEX_SUPPORTED_MODELS", raising=False)
    monkeypatch.setenv("TRADINGCODEX_MODEL_ROLLOUT", "rollback")
    with pytest.raises(ValueError, match="rollback is no longer supported"):
        make_workspace(tmp_path / "rollback")


def test_workflow_state_transitions_are_revisioned_without_lost_updates(tmp_path: Path) -> None:
    workspace = make_workspace(tmp_path)
    intake = record_workflow_intake(workspace, "NVDA company facts only.", workflow_run_id="revision-unit")
    recorded = record_workflow_plan(workspace, workflow_plan_draft(intake))
    plan = json.loads((workspace / recorded["plan_path"]).read_text(encoding="utf-8"))
...
def test_init_prepares_central_django_runtime(tmp_path: Path) -> None:
    ...
    assert "Paper Account Scope: isolated workspace paper account" in result.stdout
...
def test_starter_prompt_keeps_negated_actions_out_of_execution() -> None:
    ...
    coordinated_negation = build_workflow_intake_summary("Estimate NVDA fair value. No order or trading.")
    assert coordinated_negation["workflow_lane"] == "thesis_review"
    assert "valuation-analyst" in coordinated_negation["selected_team"]
    assert "execution-operator" not in coordinated_negation["selected_team"]
    coordinated_intent = normalize_investment_intent("Estimate NVDA fair value. No order or trading.")
    assert coordinated_intent.order_negated is True
    assert coordinated_intent.trading_negated is True
    coordinated_do_not = normalize_investment_intent("Estimate NVDA fair value. Do not order or trade.")
    assert coordinated_do_not.order_negated is True
    assert coordinated_do_not.trading_negated is True
    assert normalize_investment_intent("Analyze NVDA. No forecast.").forecast_negated is True
    assert normalize_investment_intent("Analyze NVDA without an expected return.").forecast_negated is True
    assert normalize_investment_intent("This request doesn't need valuation.").valuation_negated is True
    for descriptive_negation in (
        "The board did not recommend the transaction. Analyze NVDA.",
        "NVDA is not trading cheaply. Should I buy?",
        "Management will not sell assets. Analyze NVDA.",
        "The fund did not place an order. Analyze NVDA.",
        "NVDA is not a buy at this price. Analyze NVDA.",
        "The stock is not a sell. Analyze NVDA.",
        "This filing is not a forecast. Analyze NVDA.",
        "NVDA does not currently trade cheaply. Analyze NVDA.",
        "The fund is not actively trading NVDA. Analyze NVDA.",
        "The board did not explicitly recommend the transaction. Analyze NVDA.",
        "NVDA has no valuation premium. Analyze NVDA.",
        "The portfolio has no risk hedge. Review portfolio risk.",
        "Management issued no recommendation. Analyze NVDA.",
        "Company has no trading revenue. Analyze NVDA.",
    ):
        descriptive_intent = normalize_investment_intent(descriptive_negation)
        assert descriptive_intent.order_negated is False, descriptive_negation
        assert descriptive_intent.trading_negated is False, descriptive_negation
        assert descriptive_intent.recommendation_negated is False, descriptive_negation
        assert descriptive_intent.valuation_negated is False, descriptive_negation
        assert descriptive_intent.risk_negated is False, descriptive_negation
        descriptive_route = classify_starter_request(descriptive_negation)
        assert descriptive_route["lane"] != "order_ticket_approval_execution_gate", descriptive_negation
        assert "execution-operator" not in descriptive_route["subagents"], descriptive_negation
    assert normalize_investment_intent("Not a recommendation. Analyze NVDA.").recommendation_negated is True
    assert normalize_investment_intent("This is not a recommendation. Analyze NVDA.").recommendation_negated is True
    assert normalize_investment_intent("This request is not asking for valuation.").valuation_negated is True
    assert normalize_investment_intent("My request does not need valuation.").valuation_negated is True
    assert normalize_investment_intent("Analyze NVDA; not asking for valuation.").valuation_negated is True
    for natural_negation in (
        "I do not want orders or trading. Analyze NVDA.",
        "I am not asking for an order or trade. Analyze NVDA.",
        "I no longer want an order or execution. Analyze NVDA.",
        "Analyze NVDA without an order or a trade.",
        "Analyze NVDA. Do not place trades or submit an order.",
        "Analyze NVDA. Do not execute a trade.",
        "I do not want to execute a trade. Analyze NVDA.",
        "Analyze NVDA. Do not execute any trades.",
        "I am not trading; analyze NVDA only.",
        "I will not trade. Analyze NVDA.",
        "I will not sell. Analyze NVDA.",
        "We won't execute an order. Analyze NVDA.",
        "I cannot approve an order. Analyze NVDA.",
        "Without executing a trade, analyze NVDA.",
        "No executing trades. Analyze NVDA.",
        "I won't be trading. Analyze NVDA.",
        "I am not going to trade. Analyze NVDA.",
        "Analyze NVDA. Not a recommendation.",
        "Analyze NVDA; not asking for valuation.",
        "Trading is not requested. Analyze NVDA.",
        "Execution is not requested. Analyze NVDA.",
        "Trade execution is not wanted. Analyze NVDA.",
        "Please avoid trading and analyze NVDA.",
        "Exclude execution; analyze NVDA.",
        "Trading is prohibited; analyze NVDA.",
    ):
        routed = classify_starter_request(natural_negation)
        assert routed["lane"] != "order_ticket_approval_execution_gate", natural_negation
        assert "execution-operator" not in routed["subagents"], natural_negation
    verb_object_intake = build_workflow_intake("Analyze NVDA. Do not execute a trade.")
    assert {"trading", "execution"} <= set(verb_object_intake["explicit_negations"])
    for plural_negation, expected_flags in (
        ("Analyze NVDA. No forecasts or recommendations.", ("forecast_negated", "recommendation_negated")),
        ("Analyze NVDA. No valuations or price targets.", ("valuation_negated",)),
        ("Analyze NVDA. No charts or headlines.", ("technical_negated", "news_negated")),
        ("Analyze NVDA. No predictions or probabilities.", ("forecast_negated",)),
    ):
        plural_intent = normalize_investment_intent(plural_negation)
        assert all(getattr(plural_intent, flag) for flag in expected_flags), plural_negation
    ...
    for affirmative_execution in (
        "Execute the approved NVDA order without delay.",
        "Submit the approved paper order with no changes.",
        "Execute the approved order, no changes needed.",
        "Execute the approved order without changing its size.",
        "Without further delay, execute the approved NVDA order.",
        "No changes needed, submit the approved paper order.",
        "Without changing its size, execute the approved order.",
        "No need to wait, execute the approved order.",
    ):
        affirmative_route = classify_starter_request(affirmative_execution)
        assert affirmative_route["lane"] == "order_ticket_approval_execution_gate", affirmative_execution
        assert "execution-operator" in affirmative_route["subagents"], affirmative_execution
    draft_summary = build_workflow_intake_summary("Draft an NVDA order ticket, no risk review.")
    assert draft_summary["workflow_lane"] == "thesis_review"
    assert {"risk review", "order ticket", "approval", "execution"} <= set(draft_summary["blocked_actions"])
    assert "Stop before action" == draft_summary["review_highlights"][-1]["label"]
    draft_prompt = build_subagent_starter_prompt("Draft an NVDA order ticket, no risk review.")
    assert "Workflow lane: thesis_review" in draft_prompt
    assert "Draft-order discipline:" not in draft_prompt
    assert "Blocked actions before artifacts: portfolio review, risk review, order ticket, approval, execution" in draft_prompt
    assert "Scenario discipline: for non-valuation thesis review" in draft_prompt
    ...
    assert "Investor-context gaps" not in approved_prompt
    assert "Investor-context questions" not in approved_prompt
    ...
    assert "Investor context gaps to request before recommendation, sizing, approval, or execution" in fair_value_portfolio
    assert "Investor context questions to ask if unanswered" in fair_value_portfolio
    assert "Method lenses for this lane: Investor-context suitability gate (FINRA Rule 2111; SEC Reg BI)" in fair_value_portfolio
    ...
    assert [item["label"] for item in fair_value_summary["method_lenses"]] == [
        "Investor-context suitability gate",
        "Portfolio risk lens",
    ...
    assert fair_value_summary["next_allowed_actions"][0]["label"] == "Answer missing investor-context questions"
    ...
    broad_context = build_compact_dispatch_context("Analyze NVDA for me. No order and no trading.")
    assert broad_context["dispatch_rules"][0] == "select_record_candidate_team_before_dispatch"
    ...
    assert no_risk_draft["workflow_lane"] == "thesis_review"
    assert "portfolio-manager" not in no_risk_draft["selected_team"]
    assert "risk-manager" not in no_risk_draft["selected_team"]
    assert "risk review" in no_risk_draft["blocked_actions"]
    assert "order ticket" in no_risk_draft["blocked_actions"]
...
def test_decision_workflow_plan_and_package_are_codex_native(tmp_path: Path) -> None:
    ...
    assert "Select the smallest sufficient candidate-role team and record it before dispatch." in plan["starter_prompt"]
...
def test_dynamic_workflow_plan_validation_and_recording(tmp_path: Path) -> None:
    ...
    result = record_workflow_plan(workspace, workflow_plan_draft(intake))
    assert result["status"] == "recorded"
    assert (workspace / result["plan_path"]).exists()
    preview = json.loads((workspace / result["plan_path"]).read_text(encoding="utf-8"))
    ...
    early_body = "[factual] This judgment artifact arrived before its dependency gate opened.\n"
    early_hash = hashlib.sha256(early_body.encode("utf-8")).hexdigest()
    early_path = workspace / "trading/reports/judgment/early-judgment.md"
    early_path.parent.mkdir(parents=True, exist_ok=True)
    early_path.write_text(
        f"""---
artifact_id: early-judgment
...
content_hash: {early_hash}
input_artifact_hashes: {{}}
knowledge_cutoff: "2026-06-30"
title: Early judgment artifact
source_as_of: "2026-06-30"
readiness_label: ready-for-review
context_summary: Judgment arrived before the recorded dependency opened.
reader_summary: This artifact must not bypass stage ordering.
next_action: Wait for the evidence stage.
...
{early_body}""",
    ...
    out_of_order = evaluate_artifact_supervisor_loop(
        workspace,
        "ignored",
        ["trading/reports/judgment/early-judgment.md"],
        workflow_run_id=preview["workflow_run_id"],
    )
    early_binding = out_of_order["artifact_evaluations"][0]["binding"]
    assert early_binding["status"] == "fail"
    assert "artifact stage is not ready for acceptance" in early_binding["errors"]
    assert out_of_order["terminal_action"] == "waiting"
...
{body}""",
    ...
    record_workflow_intake(
        workspace,
        "NVDA company facts only.",
        workflow_run_id=dict_role_plan["workflow_run_id"],
    )
    ...
    dict_role_validation = validate_workflow_plan(dict_role_plan)
    assert dict_role_validation["ok"] is True
    assert dict_role_validation["roles"] == ["fundamental-analyst"]
...
def test_workflow_and_decision_cli_surfaces(tmp_path: Path) -> None:
    ...
    missing_intake_path = workspace / "missing-intake-plan.json"
    missing_intake_path.write_text(json.dumps({**preview, "workflow_run_id": "missing-cli-intake"}), encoding="utf-8")
    missing_intake = run(
        ["./tcx", "workflow", "validate", "--plan", missing_intake_path.name],
        workspace,
        expect_ok=False,
    )
    assert "recorded workflow intake is required" in missing_intake.stderr
    draft = {
        "schema_version": 1,
        "workflow_run_id": intake["workflow_run_id"],
        "selected_roles": intake["deterministic_hint"]["roles"],
    }
...
def test_workflow_validate_api_uses_url_workflow_id_for_preview(tmp_path: Path, monkeypatch) -> None:
    ...
    missing_intake_validation = client.post(
        "/api/workflows/workflow-api-smoke/validate",
        data=json.dumps({"plan": payload["deterministic_preview"]}),
        content_type="application/json",
    ).json()
    assert missing_intake_validation["ok"] is False
    assert missing_intake_validation["errors"] == ["recorded workflow intake is required"]
    record_workflow_intake(workspace, "Analyze NVDA. No valuation.", workflow_run_id="workflow-api-smoke")
    draft_validation = client.post(
        "/api/workflows/workflow-api-smoke/validate",
        data=json.dumps({
            "plan": {
                "schema_version": 1,
                "workflow_run_id": "workflow-api-smoke",
                "selected_roles": payload["deterministic_preview"]["heuristic_roles"],
            }
        }),
        content_type="application/json",
    ).json()
...
def test_subagents_prompt_cli_and_api_expose_intake_summary(tmp_path: Path, monkeypatch) -> None:
    ...
    assert cli_json["intake_summary"]["method_lenses"][0]["label"] == "Investor-context suitability gate"
    ...
    assert "investor-context gaps and artifact handoffs" in cli_json["intake_summary"]["loop_controls"][0]["detail"]
    ...
    assert "Investor context questions to ask if unanswered" in cli_json["starter_prompt"]
    ...
    assert "Investor context gap: Recommendation and sizing stay weak" in explained
    assert "Stop before action: A useful answer can end at decision support" in explained
    assert "Answer missing investor-context questions:" in explained
    assert "Method lenses:" in explained
    assert "Investor-context suitability gate:" in explained
    ...
    assert "Investor context needed before advice:" in explained
    ...
    updated_context = json.loads(run([
        "./tcx",
        "investor-context",
        "update",
        "--objective",
    ...
    assert updated_context["fields"]["investment_objective"] == "medium-term compounder review"
    assert updated_context["fields"]["time_horizon"] == "3 to 5 years"
    ...
    assert profile_aware["intake_summary"]["next_allowed_actions"][0]["label"] == "Answer remaining investor-context questions"
    assert "liquidity or cash needs" in profile_aware["intake_summary"]["next_allowed_actions"][0]["detail"]
    assert "Known workspace investor context" in profile_aware["starter_prompt"]
    ...
    saved_context = json.loads(run(["./tcx", "investor-context", "status"], workspace).stdout)
    assert saved_context["source"] == "workspace_file"
    assert saved_context["fields"]["investment_objective"] == "medium-term compounder review"
    ...
    run([
        ...
        "update",
        "--liquidity",
        "no near-term cash need",
        "--constraints",
        "taxable account, no restricted symbols",
    ], workspace)
    completed_context = json.loads(run(["./tcx", "subagents", "prompt", "--json", request], workspace).stdout)
    completed_fields = {item["field"]: item["answer"] for item in completed_context["intake_summary"]["investor_profile"]["known_fields"]}
    assert completed_fields["liquidity or cash needs"] == "no near-term cash need"
    assert completed_fields["tax, account, or jurisdiction constraints"] == "taxable account, no restricted symbols"
    assert completed_context["intake_summary"]["next_allowed_actions"][0]["detail"] == "Still missing: current holdings and concentration."
    ...
    assert payload["intake_summary"]["next_allowed_actions"][0]["label"] == "Answer remaining investor-context questions"
    ...
    run(["./tcx", "investor-context", "update", "--holdings", "cash 30%, broad equity ETF 40%, single-name ideas under 5%"], workspace)
    ...
    assert complete_profile["intake_summary"]["next_allowed_actions"][0]["label"] == "Use saved investor context"
...
def test_artifact_supervisor_loop_contract(monkeypatch, tmp_path: Path) -> None:
    ...
    recorded = record_workflow_plan(workspace, workflow_plan_draft(hook_context))
    assert recorded["status"] == "recorded"
    dynamic_plan = json.loads((workspace / recorded["plan_path"]).read_text(encoding="utf-8"))
    latest_loop_state = json.loads((workspace / ".tradingcodex" / "mainagent" / "workflow-loop-state.json").read_text(encoding="utf-8"))
    ...
    selected_roles = dynamic_plan["routing_envelope"]["required_roles"]
    assert loop_state["selected_team"] == selected_roles
    assert loop_state["allowed_followup_team"] == selected_roles
    assert loop_state["state_mode"] == "validated_dynamic_workflow_plan"
    ...
    assert plan_cli["initial_dispatch"] == selected_roles
    assert plan_cli["allowed_followup_team"] == selected_roles
    ...
    thread_a_recorded = record_workflow_plan(
        workspace,
        workflow_plan_draft(thread_a),
    )
    thread_b_recorded = record_workflow_plan(
        workspace,
        workflow_plan_draft(thread_b),
    )
...
{artifact_body}""",
    ...
    assert loop_preview["improvements"][0]["reuse_state"] == "candidate"
    ...
    recorded_loop = call_mcp_tool(
        workspace,
        "record_artifact_supervisor_loop",
        {
            "workflow_run_id": dynamic_plan["workflow_run_id"],
            "artifact_paths": ["trading/reports/news/nvda-news.md"],
        },
    ...
    assert index_payload["by_reuse_state"]["candidate"] == 1
    assert recorded_state["auto_spawn"] is False
    ...
    budget_intake = record_workflow_intake(workspace, request, workflow_run_id="budget-unit")
    budget_recorded = record_workflow_plan(workspace, workflow_plan_draft(budget_intake))
    budget_plan = json.loads((workspace / budget_recorded["plan_path"]).read_text(encoding="utf-8"))
...
{judgment_body}""",
    ...
    assert judgment_loop["pending_tasks"] == []
    assert judgment_loop["artifact_evaluations"][0]["binding"]["status"] == "fail"
    assert "artifact stage is not ready for acceptance" in judgment_loop["artifact_evaluations"][0]["binding"]["errors"]
    assert judgment_loop["terminal_action"] == "waiting"
...
def test_workspace_cli_order_policy_and_execution(tmp_path: Path) -> None:
    ...
    assert "order ticket id already exists for another paper account scope" in wrong_profile_duplicate.stderr
    wrong_profile_read = run(["./tcx", "mcp", "call", "get_order_ticket", "--ticket-id", order_id], workspace, expect_ok=False)
    assert "unknown order ticket for paper account scope" in wrong_profile_read.stderr
    ...
    assert "unknown order ticket for paper account scope" in wrong_profile_checks.stderr
    ...
    assert "unknown order ticket for paper account scope" in "\n".join(wrong_profile_submit_payload["reasons"])
...
def test_django_ninja_control_api(monkeypatch, tmp_path: Path) -> None:
    ...
    assert status["skills_installed"] == 28
    assert status["core_skills_installed"] == 28
    assert status["optional_skills_active"] >= 0
    assert status["user_visible_skills"] == ["plan-workflow", "tcx-workflow", "decision-memory", "automate-workflow", "tcx-server", "tcx-build", "investor-context", "strategy-creator"]
    ...
    assert len(client.get("/api/harness/skills?include_internal=true").json()["skills"]) == 28
...
def test_agent_skill_and_strategy_mutation_apis_feed_workbench_snapshot(tmp_path: Path, monkeypatch) -> None:
    workspace = make_workspace(tmp_path)
    monkeypatch.setenv("TRADINGCODEX_WORKSPACE_ROOT", str(workspace))
    write_optional_skill_fixture(workspace, "fundamental-analyst", "filing-red-flag-review")
    client = staff_client()
    ...
    sections = client.get("/api/workbench/").json()["sections"]
    optional_names = {item["name"] for item in sections["optional_skills"]["data"]["optional_skills"]}
    strategy_names = {item["name"] for item in sections["strategies"]["data"]}
    assert {"filing-red-flag-review", "evidence-freshness-check"} <= optional_names
    assert "strategy-catalyst-watch" in strategy_names
...
def test_workbench_research_artifact_markdown_preview(tmp_path: Path, monkeypatch) -> None:
    workspace = make_workspace(tmp_path)
    monkeypatch.setenv("TRADINGCODEX_WORKSPACE_ROOT", str(workspace))
    ...
    client = Client(REMOTE_ADDR="127.0.0.1")
    response = client.get("/api/workbench/artifacts/web-preview-note/")
    assert response.status_code == 200
    artifact = response.json()
    assert artifact["artifact_id"] == "web-preview-note"
    assert artifact["title"] == "Web Preview Note"
    assert artifact["reader_summary"] == "Plain-English preview summary."
    assert artifact["next_action"] == "Use this as evidence only."
    assert "markdown" not in artifact
    assert artifact["preview"]["heading"] == "Web Preview Note"
    assert "<h1>Web Preview Note</h1>" in artifact["preview"]["html"]
    assert "Preview body" in artifact["preview"]["html"]
    assert "<script" not in artifact["preview"]["html"]
...
def test_role_inspector_and_topology_helpers() -> None:
...
def test_file_native_research_artifacts_via_mcp_api_and_cli(tmp_path: Path, monkeypatch) -> None:
    ...
    stored = call_mcp_tool(workspace, "create_research_artifact", {
        ...
        "metadata": {"role": "fundamental-analyst"},
    ...
    assert fetched["role"] == "fundamental-analyst"
    assert fetched["producer_role"] == "fundamental-analyst"
    assert fetched["created_by"] == "fundamental-analyst"
    ...
    assert 'role: "fundamental-analyst"' in (workspace / stored["export_path"]).read_text(encoding="utf-8")
    ...
    with pytest.raises(ValueError, match="must match the authenticated MCP principal"):
        call_mcp_tool(workspace, "create_research_artifact", {
            "artifact_id": "spoofed-valuation",
            "artifact_type": "research_memo",
            "role": "valuation-analyst",
            "producer_role": "valuation-analyst",
            "created_by": "judgment-reviewer",
            "markdown": "# Spoofed producer",
        }, transport_principal="head-manager")
...
""",
    ...
    assert synthesis_quality["frontmatter"]["producer_role"] == "head-manager"
    synthesis_record = call_mcp_tool(workspace, "get_research_artifact", {"artifact_id": "synthesis-workflow-unit"})
    assert synthesis_record["created_by"] == "head-manager"
...
def test_central_db_is_shared_across_generated_workspaces(tmp_path: Path) -> None:
    ...
    assert "order ticket id already exists for another paper account scope" in order_conflict.stderr
