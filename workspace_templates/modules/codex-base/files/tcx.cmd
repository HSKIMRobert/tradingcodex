...
"{{TRADINGCODEX_PYTHON_CMD_SET}}" -c "import sys; raise SystemExit(sys.version_info.major != 3 or sys.version_info.minor not in range(11, 15))" >nul 2>nul
if not errorlevel 1 goto generated_python
where py >nul 2>nul
...
if errorlevel 1 goto check_package_runner
...
:check_package_runner
{{TRADINGCODEX_PACKAGE_RUNNER_FALLBACK_CMD}}
...
:generated_python
"{{TRADINGCODEX_PYTHON_CMD_SET}}" "%TRADINGCODEX_ROOT%.tradingcodex\cli.py" %*
exit /b %errorlevel%
...
:package_runner_launcher
{{TRADINGCODEX_PACKAGE_RUNNER_CMD}} {{TRADINGCODEX_PACKAGE_RUNNER_PREFIX_CMD}} "%TRADINGCODEX_PACKAGE_SPEC%" python "%TRADINGCODEX_ROOT%.tradingcodex\cli.py" %*
exit /b %errorlevel%
