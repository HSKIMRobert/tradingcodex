...
The main agent identity lives in `.codex/config.toml` as top-level `developer_instructions`.
Role-specific subagent identities live in `.codex/agents/*.toml` as `developer_instructions`.
...
- Treat role-owned skills as subagent harnesses. Project `.codex/config.toml` disables role-owned skills for the root/head-manager session, and each `.codex/agents/*.toml` re-enables only the skills owned by that role. `head-manager` may inspect and assign analyst, portfolio, risk, approval, and execution skills, but must not use those skills to perform the role work directly.
...
- Give subagents role-local context, not the full product context: affiliation, coordinator, assigned role, role purpose, own artifacts, handoff target, and forbidden actions.
