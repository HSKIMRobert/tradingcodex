...
Build operation. Proceed only when the current root prompt has the exact
physical first line `$tcx-build`, followed by a non-empty request that invokes
or clearly requests `$tcx-strategy`. Plan mode is not Build authority.
...
```text
$tcx-build
Use $tcx-strategy to <create, update, activate, archive, or delete> <strategy-name> with <reviewed request>.
```

Build admission remains bounded by the active Codex sandbox, the deterministic
hook, workspace-only paths, and the shared strategy service. It is not order,
approval, secret, or global-configuration authority.

## Workflow

1. Verify Build admission before any tool use. Identify whether the explicit
...
3. Use `$skill-creator` only to draft a concise body, then apply the strategy
   boundaries in this skill. The shared strategy service remains the exclusive
...
5. For each durable operation, invoke only its corresponding trusted workspace
   service command:

   ```text
   {{TRADINGCODEX_WORKSPACE_LAUNCHER}} strategies create <strategy-name> --description <description> --body-file <workspace-body-file> --language <language> --status <draft|active>
   {{TRADINGCODEX_WORKSPACE_LAUNCHER}} strategies update <strategy-name> --description <description> --body-file <workspace-body-file> --language <language> --status <draft|active>
   {{TRADINGCODEX_WORKSPACE_LAUNCHER}} strategies activate <strategy-name>
   {{TRADINGCODEX_WORKSPACE_LAUNCHER}} strategies archive <strategy-name>
   {{TRADINGCODEX_WORKSPACE_LAUNCHER}} strategies delete <strategy-name>
   ```

   For create or update, pass the reviewed body with `--body-file`, plus an
...
7. Inspect the service result and, within the admitted Build turn, validate with
   `{{TRADINGCODEX_WORKSPACE_LAUNCHER}} strategies inspect <strategy-name>` and
   `{{TRADINGCODEX_WORKSPACE_LAUNCHER}} strategies list`. Treat service-created
...
8. For an evidence-driven change, show the old and proposed rules, reason,
   ...
   exact `$tcx-build` root turn that explicitly approves the reviewed update;
   never carry the prior turn's Build grant across the reply.
